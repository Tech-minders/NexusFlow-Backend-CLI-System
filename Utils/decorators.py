...
def require_login(_func=None, **_kwargs):
    #Reads the logged-in user from self.session.current_user
    def decorator(func):
        @functools.wraps(func) 
        def wrapper(self,*args, **kwargs):
            current_user = getattr(getattr(self, "session", None), "current_user", None)
...
def log_action(action_name:str):
    ...
    def decorator(func):   # receives the actual function being decorated
        ...
        def wrapper(self,*args, **kwargs):
            ...
            user      = getattr(getattr(self, "session", None), "current_user", None)
